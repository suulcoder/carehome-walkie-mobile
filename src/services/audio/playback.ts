 ...
 * - Missing chunks: only after CHUNK_ARRIVAL_GRACE_MS (5 s) post-ptt_end; live segments never gap-fill.
 */
...
import { isCaptureActive, waitForCaptureIdle, WALKIE_RECORDING_AUDIO_MODE } from "./capture";
...
import { isSameDisplayName } from "../websocket/peers";
...
interface ReceiveSession {
  ...
  /** Set when CHUNK_ARRIVAL_GRACE_MS elapses after ptt_end — only then may missing seqs be filled with silence. */
...
function hasAllExpectedChunks(session: ReceiveSession): boolean {
  ...
  const seqs = new Set(session.chunks.map((c) => c.seq));
  if (seqs.size < expected) return false;
...
function gapFillAllowed(session: ReceiveSession): boolean {
  return session.endReceived && session.graceExpired;
}
...
function selectPlaybackRange(session: ReceiveSession, mode: PlayMode): SegmentSelection | null {
  ...
  if (mode === "final_partial") {
    const useGapFill = gapFillAllowed(session) && session.expectedCount !== undefined;
    if (useGapFill) {
      const endSeq = session.expectedCount! - 1;
      if (endSeq < session.nextPlaySeq) return null;
  ...
  // Live segments never gap-fill: ptt_end often beats late chunks on the wire.
  const drainAllContiguous = session.streamingPlayback || session.nextPlaySeq > 0;
  let totalBytes = 0;
...
function schedulePlaybackAttempt(sessionId: string): void {
  ...
  session.graceTimer = setTimeout(() => {
    ...
    if (remainingMs === 0 && hasAllExpectedChunks(session)) {
      finishSession(sessionId, session, true);
      return;
    }
    ...
    telemetry.warn("playback", "chunk_grace_timeout", {
      ...
      message: "Grace window elapsed — playing with gap fill for any missing seqs",
    });
    enqueuePlayback(sessionId, "final_partial");
  }, CHUNK_ARRIVAL_GRACE_MS);
}
...
export function replayStoredMessage(message: StoredMessage): Promise<void> {
  const existing = pendingPlayQueue.find(
    (item) => item.kind === "stored" && item.message.sessionId === message.sessionId
  );
  if (existing?.kind === "stored") {
    return new Promise((resolve, reject) => {
      const prior = existing.settle;
      existing.settle = (result) => {
        prior?.(result);
        if (result.ok) resolve();
        else reject(new Error(result.error ?? "playback failed"));
      };
    });
  }

  return new Promise((resolve, reject) => {
    pendingPlayQueue.push({
      kind: "stored",
      message,
      settle: (result) => {
        if (result.ok) resolve();
...
async function activatePlaybackSession(): Promise<void> {
  if (playbackSessionActive && livePlaybackDepth > 0) return;
  await waitForCaptureIdle();
  if (isCaptureActive()) {
    throw new Error("microphone active — release PTT before replay");
  }
...
async function writeWavToCache(sessionId: string, wavBytes: Uint8Array, suffix?: string): Promise<File> {
  ...
  if (!file.exists || file.size < 44) {
    throw new Error(`wav cache write failed (${file.exists ? file.size : 0} bytes)`);
  }
  return file;
}
...
async function waitForPlaybackFinish(
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      cleanup();
      telemetry.warn("playback", "load_timeout", {
        sessionId,
        data: {
          timeoutMs,
...
async function playAudioPayload(input: PlayAudioPayloadInput): Promise<boolean> {
  ...
  try {
    ...
    if (peakAmplitude < SILENT_PEAK_THRESHOLD && gapsFilled === 0) {
    ...
    } else if (peakAmplitude < SILENT_PEAK_THRESHOLD && gapsFilled > 0) {
      telemetry.debug("playback", "gap_fill_silent_segment", {
        sessionId,
        data: { peakAmplitude, expected: chunkCount, playMode, gapsFilled },
      });
    }
