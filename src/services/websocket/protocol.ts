...
// Shared protocol types — keep in sync with server repo's server/src/types/protocol.ts
...
export type ClientMessage =
  ...
  | {
      ...
      sessionId: string;
      seq: number;
      /** PCM16 or Opus frame bytes, depending on `codec`. */
      pcmBase64: string;
      codec?: AudioCodec;
    }
...
/** Serializable history row from server history_sync — maps to ServerHistoryEntry */
